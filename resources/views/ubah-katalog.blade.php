  ...
  <section id="catalogue" class="py-30 text-black">
    <div class="container mx-auto px-4">
      ...
      <div class="flex flex-col rounded-md shadow-md overflow-hidden px-4 md:px-10 py-7 md:py-10">
        <a href="/katalog" class="flex items-center hover:font-bold group transition-all duration-200">
          <span class="material-symbols-outlined mr-1 group-hover:-translate-x-3 transition-all duration-200 ease-in-out">arrow_back</span>Kembali ke Katalog
        </a>
        <h2 class="font-bold text-xl mt-10">Ubah Data Produk</h2>
        <form action="/katalog/{{ $medicine->slug }}/ubah" method="POST" enctype="multipart/form-data" class="flex flex-col md:flex-row w-full">
          @method('PATCH')
          <div class="flex flex-col w-full md:w-1/3">
            ...
            <div class="grid grid-cols-2 mt-5 gap-5">
              <div class="flex flex-col">
                ...
                <div class="flex items-center">
                  ...
                  <input type="text" name="price" id="price" placeholder="10000" value="{{ old('price', $medicine->price) }}" class="outline-none mt-1 py-1 border-b-1 text-sm focus:border-b-2 focus:border-orange w-full" required>
                </div>
                @error('price')
                  <p class="text-red text-sm mt-1 font-bold">{{ $message }}</p>
                @enderror
              ...
              <div class="flex flex-col">
                ...
                <input type="number" name="stock" id="stock" placeholder="50" value="{{ old('stock', $medicine->stock) }}" class="outline-none mt-1 py-1 border-b-1 text-sm focus:border-b-2 focus:border-orange w-full" required>
                @error('stock')
                  <p class="text-red text-sm mt-1 font-bold">{{ $message }}</p>
                @enderror
              </div>
            ...
            <div class="flex flex-col mt-5">
              ...
              <div id="description-container">
                @foreach($medicine_description as $description)
                  <div class="description-input mb-3 flex items-center gap-2">
                    <input type="text" name="description[]" value="{{ $description->description }}" class="outline-none mt-1 py-1 border-b-1 text-sm focus:border-b-2 focus:border-orange w-full" required>
              ...
              <button type="button" class="px-3 py-1 bg-gray-200 rounded-md text-sm hover:bg-gray-300 w-fit flex items-center cursor-pointer" id="add-description-btn">
                <span class="material-symbols-outlined text-sm">add</span> Tambah Deskripsi
              </button>
            </div>
            <div class="flex flex-col mt-5">
          ...
          <div class="img-preview-container flex-col w-full md:w-1/2 mt-5 md:mt-0 md:ml-12 {{ $medicine->image ? 'flex' : 'hidden' }}">
            <h3 class="font-bold mb-3">Pratinjau Gambar</h3>
            <div class="flex items-center">
              <img class="img-preview object-contain rounded-lg shadow-sm max-h-[500px]" src="{{ $medicine->image ? asset('storage/' . $medicine->image) : '' }}" alt="Preview">
            </div>
          </div>
        </form>
      </div>
