          ...
          <div class="container mx-auto px-4">
               ...
               <div class="flex flex-col rounded-md shadow-md overflow-hidden px-4 md:px-10 py-7 md:py-10">
                    ...
                    <div class="flex justify-between mt-7">
                         <div class="flex flex-col w-3/4">
                              ...
                              <ul class="flex flex-col mt-4">
                                   ...
                                   @foreach ($categories as $category)
                                        <li class="py-1.5 px-3 border-1 border-orange rounded-lg mb-3 flex items-center justify-between">
                                             <div class="flex items-center">
                                                  <h3 class="font-bold">{{ $category->name }}</h3>
                                                  <p class="text-xs text-gray-500 ml-5 font-medium">Jumlah Produk: {{ $medicines->where('category_id', $category->id)->count() }}</p>
                                             </div>
                                             <div class="flex items-center">
                                               <div x-data="{ editing: false }">
                                                       <form x-show="editing" 
                                                              ...
                                                              <input type="text" 
                                                                         name="name" 
                                                                         value="{{ $category->name }}"
                                                                         class="outline-none py-1 px-2 rounded border-1 border-orange focus:border-orange focus:bg-orange/5">
                                                       </form>
                         ...
                         <div class="flex flex-col border-1 border-orange rounded-lg py-4 px-6 h-fit">
                              ...
                              <form action="/katalog/kategori" method="POST" class="flex items-center mt-3">
                                   @csrf
                                   @method('POST')
                                   <input type="text" name="name" placeholder="Nama Kategori" class="outline-none mt-1 py-1.5 px-3 rounded-md border-1 text-sm focus:border-orange focus:bg-orange/5">
