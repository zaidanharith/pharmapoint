    ...
    <section id="catalogue" class="py-30 text-black">
      <div class="container mx-auto px-4">
        ...
        <div class="flex flex-col rounded-md shadow-md overflow-hidden px-4 md:px-10 py-7 md:py-10">
          ...
          <form action="/katalog/tambah" method="POST" enctype="multipart/form-data" class="flex flex-col md:flex-row w-full">
         <div class="flex flex-col w-full md:w-1/3">
          ...
          @enderror
           ...
           <div class="flex flex-col sm:flex-row mt-5 items-start sm:items-center gap-5 sm:gap-0">
          <div class="flex flex-col w-full sm:w-auto">
            ...
            <div class="flex items-center">
              ...
              <input type="text" name="price" id="price" placeholder="10000" value="{{ old('price') }}" class="outline-none mt-1 py-1 border-b-1 text-sm focus:border-b-2 focus:border-orange w-full" required>
            </div>
            @error('price')
              <p class="text-red text-sm mt-1 font-bold">{{ $message }}</p>
            @enderror
          </div>
          <div class="flex flex-col w-full sm:w-auto sm:ml-5">
            <label for="stock" class="font-medium after:content-['*'] after:text-red after:ml-1">Jumlah Stok</label>
            <input type="number" name="stock" id="stock" placeholder="50" value="{{ old('stock') }}" class="outline-none mt-1 py-1 border-b-1 text-sm focus:border-b-2 focus:border-orange w-full" required>
            @error('stock')
              <p class="text-red text-sm mt-1 font-bold">{{ $message }}</p>
            @enderror
          </div>
         ...
         <div class="img-preview-container flex-col w-full md:w-1/2 mt-5 md:ml-12 hidden">
           <h3 class="font-bold mb-3">Pratinjau Gambar</h3>
           <div class="flex items-center">
            <img class="img-preview object-contain rounded-lg shadow-sm w-full" alt="Preview">
           </div>
         </div>
          </form>
        </div>
