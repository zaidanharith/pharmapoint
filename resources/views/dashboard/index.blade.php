<x-layout>
    ...
    <section id="catalogue" class="py-30 text-black">
        <div class="container mx-auto px-4">
            <div class="flex flex-col md:flex-row justify-between mb-5 items-start md:items-center gap-3">
                @php
                ...
                <div class="flex items-center">
                    <h1 class="font-extrabold text-2xl md:text-4xl text-blue-dark">{{ $greeting }}, {{ auth()->user()->name }}</h1>
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center rounded-md shadow-md overflow-hidden px-4 md:px-5 py-3 mb-5 gap-4">
                <p class="font-bold text-md">Halo! Bagaimana kabarmu hari ini?</p>
                <div class="flex flex-col items-start md:items-end text-left md:text-right w-full md:w-auto">
                    <div class="flex flex-col mb-3 w-full md:w-auto">
                        <a href="/dashboard/{{ auth()->user()->username }}" class="py-1.5 px-3 rounded-md font-medium text-white bg-blue-dark text-sm cursor-pointer hover:bg-blue-dark/90 flex items-center w-full md:w-auto justify-center md:justify-start">
                            <span class="material-symbols-outlined mr-2">edit</span>Ubah Profil
                        </a>
                    </div>
                    <div class="flex flex-col items-start md:items-end text-left md:text-right w-full">
                    @if(!auth()->user()->is_admin && !auth()->user()->request_admin && !auth()->user()->is_owner)
                        <form action="{{ route('dashboard.requestAdmin', auth()->user()) }}" method="POST" class="flex items-center w-full md:w-auto">
                            @csrf
                            <button type="submit" name="action" value="request" class="w-full md:w-auto bg-green-600 text-white font-medium px-3 py-1.5 rounded-md hover:bg-green-700 text-sm cursor-pointer">
                    ...
                    @if ($transactions->count())
                        @foreach ($transactions as $transaction)
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-center rounded-lg border-1 border-gray-300 px-4 py-2 mb-1 gap-2">
                                ...
                                <p class="font-medium text-sm">{{ $transaction->created_at->format('d M Y, H:i:s') }}</p>
                            </div>
                        @endforeach
                    @else
                    ...
                    <div class="flex flex-col mt-5 gap-y-3">
                        @if ($users->contains('request_admin', true))
                            @foreach ($users as $user)
                                ...
                                <div class="flex flex-col md:flex-row justify-between items-start md:items-center px-4 py-2 rounded-lg mb-1 border-1 border-orange gap-3">
                                    <div class="flex flex-col">
                                        <h2 class="font-bold text-lg">{{ $user->name }}</h2>
                                        <p class="text-gray-500 text-xs">{{ $user->updated_at->format('d M Y, H:i') }}</p>
                                    </div>
                                    <form action="{{ route('dashboard.approve', $user->id) }}" method="POST" class="flex items-center w-full md:w-auto gap-2">
                                        @csrf
                                        <button type="submit" name="action" value="approve" class="w-full md:w-auto flex items-center justify-center rounded-md bg-green-500 font-medium px-3 py-1.5 cursor-pointer hover:bg-green-400">
                                            <span class="material-symbols-outlined mr-1">check</span>Terima
                                        </button>
                                        <button type="submit" name="action" value="reject" class="w-full md:w-auto flex items-center justify-center rounded-md bg-red-500 font-medium px-3 py-1.5 cursor-pointer hover:bg-red-400 text-white">
                ...
                <div class="mt-5">
                    ...
                    @foreach ($medicines->sortBy('stock') as $medicine)
                        @if ($medicine->stock <= 10 && $medicine->stock > 0)
                            <a href="/katalog/{{ $medicine->slug }}" class="flex flex-col md:flex-row justify-between items-start md:items-center px-3 py-1.5 rounded-lg mb-3 border-1 border-orange cursor-pointer hover:bg-orange/10 transition-all duration-200">
                                <div class="flex flex-col md:flex-row items-start md:items-center justify-between w-full gap-2">
                                    <div class="flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
                                        <h2 class="font-bold text-red">{{ $medicine->name }}</h2>
                                        <p class="text-gray-500 text-xs">{{ $medicine->category->name }}</p>
                                    </div>
