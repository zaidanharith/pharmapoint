<x-layout>
    ...
    <section id="catalogue" class="py-30 text-black">
        <div class="container mx-auto px-4">
           <div class="flex flex-col md:flex-row md:justify-between mb-5 md:items-center gap-3">
              @php
           ...
           <div class="flex flex-col rounded-md shadow-md overflow-hidden px-4 py-7 md:p-10">
             <a href="/dashboard" class="flex items-center hover:font-bold group transition-all duration-200 w-fit">
                <span class="material-symbols-outlined mr-1 group-hover:-translate-x-3 transition-all duration-200 ease-in-out">arrow_back</span>
                Kembali ke Dashboard
             </a>
             
             <div class="flex flex-col lg:flex-row justify-between mt-5 gap-5">
                <div class="flex flex-col w-full lg:w-1/2">
                    ...
                    <div class="flex flex-col rounded-lg border-1 border-gray-300 overflow-hidden">
                       @foreach ($carts as $cart)
                          <a href="/katalog/{{$cart->medicine->slug}}" class="p-4 flex flex-col sm:flex-row sm:items-center justify-between cursor-pointer hover:bg-orange/5 transition-all duration-200 gap-3">
                             <div class="flex flex-col sm:flex-row sm:items-center gap-2">
                                <h4 class="font-bold">{{$cart->medicine->name}}</h4>
                                <p class="text-sm font-medium text-gray-500 sm:ml-5">Rp{{ number_format($cart->medicine->price, 0, ',', '.') }} <span class="ml-1">x {{$cart->medicine_quantity}}</span></p>
                             </div>
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-center mt-5 rounded-lg border-2 border-orange p-4 gap-3">
                ...
                <div class="flex flex-col rounded-lg border-1 border-orange p-5 items-center h-fit w-full lg:w-1/3">
                    ...
                    <form action="/dashboard/keranjang/beli" method="POST" class="flex flex-col w-full">
                       ...
                       <button type="submit" class="bg-orange text-blue-dark font-bold py-2 px-4 rounded-md mt-5 hover:bg-orange/90 transition-all duration-200 flex justify-center items-center cursor-pointer">
